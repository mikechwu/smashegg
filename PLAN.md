...
**Multi-tab policy:** one live socket per seat — a new authenticated socket for a seat closes the old one (tag lookup). Simplest correct MVP behavior.
...
- Player identity = per-join random 128-bit token, hashed (SHA-256) at rest in the DO; possession of the token = the seat. No accounts in MVP.
...
Auto-iteration loop within each milestone: implement → tests → self-review → **cross-model audit panel** → STATUS.md update. Panel composition (all three CLIs verified installed on this machine): **Codex CLI** — logic and security, adversarial reads of the engine and protocol; **Gemini CLI** — large-context passes over the whole spec/repo at once; **Grok CLI** — optional third lineage for corroborating research findings and rules claims, added where it earns its cost under the budget policy. Each pass and gate records in STATUS.md which models ran and why; a load-bearing check never relies on same-family self-review alone (lineage diversity is the point — historically that's where the catches come from). A gate passes only with everything green; load-bearing audit findings go to you for confirmation.

Round-3 tracked item: the `aFailConsequence` level-selection module (suspend/resume, spec §1.5/§6.4) is engine work — lands in M1, must be complete and tested before the M3 gate. Not an M0 blocker.
...
| **M2** | GameRoom DO proven with `guess-number`: connect, broadcast, turn loop, hibernation, alarms, resync skeleton; **room-dump route + `scripts/dump-room.ts`**; structured mutation logs | 4 simulated clients complete a game via `wrangler dev`; DO code imports only `engine/core`; kill/restart DO mid-game → clients resume from snapshot; **dump→replay roundtrip: a room dump feeds the replay harness and reproduces the DO's snapshot**; **G-ALARM (hibernated)** and **G-WSMETER** pass |
