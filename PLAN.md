...
- **G-SIM** (standing, new 2026-07-27, **replaces G-FOLD for the phone**): **a change must not reduce the worst-case SIMULTANEITY SLACK below the recorded baseline for that `viewport@theme`.** Slack is `innerH − span`, where span is the union DOCUMENT extent of the must-see facts; a negative slack means no scroll offset shows them all, so the auto-scroll cannot rescue it. Reported by `scripts/measure-simultaneity.mjs`.
  ...
  **The must-see set is a DEFINITION, and it is not settled** (METHODOLOGY practice 28). The gate therefore computes three declared profiles from one run and prints them side by side: `in-house` (adds the level badges and opponents' card counts), `panel` (Codex and Grok, independently converged: trick well, play desk, fan, action bar), `minimal` (desk + actions). **Pass/fail is gated on `minimal`** until the owner rules, because failing the weakest defensible definition cannot be argued away by re-litigating the set.
  **Recorded baselines (lacquer, no shelf, worst case over the sample — a floor, so worst not median):**

  | inner | in-house | panel | minimal | n |
  |---|---|---|---|---|
  | 390×664 | −113.2px (infeasible 87.5%) | **+55.0px** (0%) | +513.5px (0%) | 24 |
  | 390×748 | −50.5px (infeasible 33.3%) | **+117.7px** (0%) | +597.5px (0%) | 24 |
  | 1366×681 | **+26.3px** (0%) | +179.1px (0%) | +530.5px (0%) | 16 |
  | 1440×813 | +158.3px (0%) | +311.1px (0%) | +662.5px (0%) | 16 |

  These are worst-observed, **not structural minima** — a thin margin against a rare hand is the shape that fails in the field (practice 14's corollary), and 26.3px at 1366×681 is thin. A shelf open costs a further ~140px of span and is measured separately.
