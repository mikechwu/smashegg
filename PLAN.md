...
- `S`: team levels, per-seat hands (multisets), phase (`ceremonyCut | tribute | returnTribute | playing | matchEnd` — no actorless phases: `ceremonyCut` is hand 1's REAL cut under `firstLeadMethod='drawCard'`, with the PRNG-uniform cutter as its actor and a committed shuffled `deck` in state, hidden exactly like the PRNG; the hand-ending `applyAction` atomically scores, deals hand N+1 from the stored PRNG state, and enters the next hand's tribute/playing phase), current trick (top play + declared combo + passes), finish order, tribute bookkeeping, per-seat `actedThisHand` flags (the per-seat planning window), variant config, hand number, PRNG state.
- `A`: `{type:'play', cards, decl}` (decl = declared canonical combination — required when wilds make ≥2 interpretations valid, per spec §4.4), `{type:'pass'}`, `{type:'payTribute', card}` *(a **choice**: the rank is forced — highest by levelValue in the newly dealt hand, heart-level wilds excluded — but when several cards share that rank the payer picks the concrete card; suit choice is strategic, e.g. preserving a straight-flush suit. `legalActions` returns the exact eligible set; `applyAction` validates membership in it, not equality to one precomputed card)*, `{type:'returnTribute', card}` *(same choice treatment over the eligible `levelValue ≤ 10` set)*, `{type:'cutDeck', position}` *(the REAL hand-1 cut: interior positions 6..102, exact-set `legalActions`, defaultAction = the middle cut. Ceremony-marker round 2026-07-15 — REVERSED geometry, superseding "flips and the deal derive from the same rotated deck": the cut PRESERVES deck order — lift, look at the split, put back — and selects the revealed cards: under `ceremonyCardCount: 2` (owner default) the count card is the lifted packet's bottom and the face-up the marker card marker is the table packet's top; `1` is the official one-card form. The cut moves WHO LEADS (the marker's deal depth), never which cards a seat group holds — still genuine agency — and replay reproduces it from the logged action)*. Anti-tribute needs no action in `auto` mode (`antiTributeMode`, spec §7.6); an explicit decision action exists only under the `optional` variant.
...
**Hibernation discipline:** `ctx.acceptWebSocket(ws)` with **no tags** (a socket's seats are unknown at upgrade time — it acquires them later via hello tokens / claimSeat); per-socket `serializeAttachment({seats: Seat[], acceptedAt})` (seats: a SET — one connection can hold 0–4 seats in the multi-seat/self-play model; ≤16KB limit); constructor rehydrates the socket→seats map from `getWebSockets()+deserializeAttachment()`; in-memory maps are rebuildable caches, never authoritative; `setWebSocketAutoResponse('ping'→'pong')` for zero-cost keepalive — note this matches **exact literal strings only**, so clients send the bare string `ping` *outside* the JSON envelope (a JSON-wrapped ping would wake the DO and be billed). The edge-answered ping is also the DO's only HUMAN-liveness signal (socket-liveness.md: locked/frozen phones stop pinging without a close frame, and nothing else ever closes their sockets — measured), so on every wake the DO sweeps `getWebSocketAutoResponseTimestamp(ws)` (readable without waking) and CLOSES any socket silent ≥ `STALE_SOCKET_MS` (3 min; > Chrome's ~60s background-throttled cadence) — the ordinary disconnect machinery (presence → grace → Q3 pause → retention re-arm) runs from there, and a staleness-sweep alarm candidate guarantees such a wake exists whenever any socket is attached. Duration billing stops while hibernated (verified). `compatibility_date` will be ≥ 2026-04-07, which enables `web_socket_auto_reply_to_close`: the runtime auto-replies to Close frames, so `webSocketClose` must not assume it owes a close handshake.

**Turn timeouts (DO Alarms):** on every DECISION point (state change) recompute `deadlines` (one row per expected actor; a *newly*-acting seat gets a fresh budget in `base_due_at` — the ONLY fresh-clock path — resolved PER SEAT since the design-refinement round: each seat's class (`timingClass(state, seat)`) maps through RoomTiming, so co-actors can hold different budgets (a still-planning tribute payer vs one who committed); a seat that remains an actor keeps its base; a disconnected actor clamps toward `now + disconnectGraceMs`, and `null` `actionTimeoutMs` disables the clock only for connected seats), then `setAlarm(min(due_at))`. Presence changes may only clamp toward the grace or RESTORE up to the preserved base — never re-arm a fresh clock (the M4 fix — room-timing.md §2; PLAN's earlier "recompute `now + actionTimeoutMs` every transition" was the pre-M4 bug and is corrected here). `alarm()` re-reads deadlines (stale-alarm guard) and applies `defaultAction` for each overdue seat via the normal action path — **but only while a seat is connected** (Q3 pause: a room with zero connected seats is frozen, no auto-play, so no rows-written burn accrues; pause-and-retention.md). Alarms wake hibernated DOs and survive eviction (verified). **Retention TTL:** the single alarm slot also carries a per-room self-purge — an ABANDONED room (zero live *sockets*) past its retention window `deleteAll()`s itself. In the default LAZY mode only lobby-abandoned rooms auto-purge (a few rows, cheap); played-out/paused rooms arm no TTL and are reclaimed manually via `scripts/cleanup-rooms.ts` (rows-written is the scarce, fail-closed meter; storage is abundant — auto-purging a big room would spend scarce to reclaim abundant). The null-timeout-while-disconnected case is part of the M4 deadlock-freedom property test.
...
**Lobby phase (owner direction, M2; extended by the design-refinement round):** rooms start in a pre-game LOBBY: seats are claimed (each claim mints that seat's token; an optional `seat` param picks a SPECIFIC empty seat — the lost race rejects `seat.taken`), RELEASABLE while in the lobby (release deletes the seat row — invalidating the token for every holder — and purges the seat from every socket's delivery map; the next claim mints fresh; mid-match release is refused: a seat owns a dealt hand, and leaving is modelled by the disconnect machinery), and RENAMABLE anytime (names are cosmetic; the engine never sees them). The room's `{gameId, config}` — **opaque game-defined data to the room layer** — is editable with live `configChanged` broadcasts. Chosen default (simple, family-friendly; owner delegated the detail): **any seated player may edit the config and any seated player may start once all seats are claimed**; start runs `Game.init` (a guarded/invalid config fails the start with a semantic error and the room stays in lobby), then config is **frozen for the match**. The M3 Guandan lobby UI surfaces a curated subset of the 25 RuleVariant keys over this transport; the room layer never learns any Guandan key.
...
{ v:1, type:'claimSeat', name, seat? }               // lobby; seat? = choose-your-seat (omitted = first free)
{ v:1, type:'releaseSeat', seat }                    // lobby, holder-only — INVALIDATES the seat's token
                                                      // (row-level hash delete + delivery purge; the
                                                      // redaction hard line); next claim mints fresh
{ v:1, type:'renameSeat', seat, name }               // anytime, holder-only — names are cosmetic
...
{ v:1, type:'seatReleased', seq, seat }              // the seat's token is DEAD; clients drop the credential
...
- **G-SIM** (standing, new 2026-07-27, **replaces G-FOLD for the phone**): **a change must not reduce the worst-case SIMULTANEITY SLACK below the recorded baseline for that `viewport@theme`.** Slack is `innerH − span`, where span is the union DOCUMENT extent of the must-see facts; a negative slack means no scroll offset shows them all, so the auto-scroll cannot rescue it. Reported by `scripts/measure-simultaneity.mjs`.
  **Why this replaces G-FOLD on the phone.** The fold rate at a real inner 390×664 is 100% for every candidate layout, so it ranks nothing — a metric with no gradient cannot gate. Slack has a gradient measured in pixels, which is the unit design changes are actually denominated in: a 24px→44px indicator costs exactly +20px of span, derivable rather than sampled.
  **The must-see set is a DEFINITION** (METHODOLOGY practice 28). The gate computes three declared profiles from one run and prints them side by side: `in-house` (adds the level badges and opponents' card counts), `panel` (Codex and Grok, independently converged: trick well, play desk, fan, action bar), `minimal` (desk + actions). **OWNER RULING 2026-07-27: G-SIM is stated against the `panel` set**, on Grok's structural argument that a set containing `.gd-headline` drags the span back to "everything above the fold" and silently rebuilds the metric being replaced. All three stay printed so a definition change is visible rather than silent. The gate's automatic pass/fail remains on `minimal` (unarguable under any definition); the `panel` row is the one the owner rules on.
  **The level's absence from the set is a DESIGN DEFECT with a named fix, not an accepted cost** — 打幾 governs every rank comparison and elders are the population most likely to re-check a constant. The fix is a level chip **inside `.gd-desk`**, which the panel set already requires visible: measured 2026-07-27, a chip up to **84.4px wide and ≤27px tall costs ZERO span** (the title row is 27px, set by the title, not by the clock; measured against the longest own-turn title with a clock element present). Both properties, no trade.

  **Recorded baselines — lacquer, no shelf, WITH ONE CARD STAGED, worst case over the sample.** Staged, because that is the state the player decides IN: staging opens `.gd-desk__stage`, worth **+54.0px** of span, and it saturates at the FIRST card (a non-wrapping flex row capped at `DESK_STAGE_MAX_FACES = 10`, verified 0..12). Every figure recorded before this was un-staged and therefore described a state the player only passes through.

  | inner | in-house | **panel (the gated set)** | minimal | n |
  |---|---|---|---|---|
  | 390×664 | −188.5px (100%) | **−20.3px (infeasible 4.2% [0.7, 20.2]; 12.5% not all visible at the settled scroll)** | +459.5px (0%) | 24 |
  | 390×748 | −83.2px (100%) | **+85.0px (0%)** | +543.5px (0%) | 24 |
  | 1366×681 | −81.4px (81.3%) | **+71.4px (0%)** | +451.4px (0%) | 16 |

  **The span is fully DERIVED as of 2026-07-27** — no term is sampled any more.
  `span = fanHeight + deskHeight + K`, each term proved:
  - **K = 198.6px following** (the trick well renders) / **66.0px leading** (well empty, 0×0), constant to 0.1px across 78 measured states. The 132.5px difference is 73.5px of well plus a 59px band — and that band is **occupied by the west/east seat plates and stacks**, which reach to 367.7 against a fan top of 375.7, so only ~8px of it is empty. Within the 66.0px: 10px fan→desk, 15px desk→actions, 41px the action bar itself. **Pure recoverable spacing in the entire span is ~33px**, and taking all of it would collapse Play/Pass onto the desk and the fan onto the ring. There is no cheap 20px.
  - **deskHeight = 156.5px** (timed, staged), single-valued again as of 2026-07-27. The 161.5px second state was **a layout defect, now fixed, not a property of the joker face.** `.gd-cardframe` is `inline-flex` and sat on its parent's text baseline, so the parent's line box reserved the font's descender below it; a card with no in-flow text (a joker — its art is `position: absolute`) has its baseline at its bottom margin edge, so the descender landed entirely below the card and added ~5px. Proved by intervention: `vertical-align: top` or `display: block` on the frame took the staged-card button from 78.5 to 73.5. **Fixed with `vertical-align: top` on `.gd-cardframe`**, pinned by `play-desk.test.tsx`, and verified: **156.5 across 28 value classes including both jokers**, containment green.
    ***The earlier claim that a joker face has aspect 1.548 is WITHDRAWN.*** `.gd-card` is `calc(var(--gd-cardw) * 1.45)` and measures 73.5 in the fan, the well and the stage alike; the joker's SVG is 69.5px, absolutely positioned inside it with 4px of slack — **not squashed, not cropped.**
  - **K itself scales with the card (D2).** The trick well renders `<CardFace size="hand">` (`TrickWell.tsx:60,67`), selecting the same `clamp(2.75rem, 13vw, 4.25rem)` as the fan — `.gd-card--trick` is documented dormant. Measured: well card 50.7×73.5, fan card 50.7×73.5. So `K = 125.1 + 1.45·cardW`, and a card-scale change removes **8.3px** from K at cardW 44.95 that the threshold model never counted.
...
- **G-FOLD** (standing, restated 2026-07-27; **VOID for the phone, live for desktop**): **a change must not raise the below-fold rate above the measured no-shelf BASELINE for that viewport** — it is not "Play is never below the fold", which the product has never satisfied and was never going to. The phone baseline of **8.3% [3.3%, 19.6%] (n=48, lacquer)** is **VOID**: it was measured at inner 390×844, a phone SCREEN size no browser presents, and at a real 390×664 or 390×748 the rate is 100% at every pile depth. The row survives in `measure-fold.mjs` with `void: true` for provenance, and the gate now **exits 4** rather than reporting a comparison against it. Desktop modes are 0/24 after rung 0 and keep this gate. Every decision previously taken against the phone baseline is OPEN, not resolved (METHODOLOGY practice 27).
  **Baselines are keyed by `viewport@theme`, and that is not bookkeeping.** `stackStripW` is a per-theme metric driving pile height, and the second shipping theme measured 95.8% below fold on a phone with no shelf against lacquer's 4.2%. An accepted rate for one theme says nothing about another; the gate refuses to compare across them. Reported by `scripts/measure-fold.mjs` as a rate with its Wilson interval, its sample size and its INNER viewport dimensions, printed beside the baseline so the comparison is in the instrument rather than in someone's memory.
...
| **M3** | Guandan behind the interface; table UI (usable, zh-Hant default) | 4 browser clients finish a full match locally (tribute + upgrade + low-card alert visible); e2e test drives a scripted full hand |
