...
- **G-SIM** (standing, new 2026-07-27, **replaces G-FOLD for the phone**): **a change must not reduce the worst-case SIMULTANEITY SLACK below the recorded baseline for that `viewport@theme`.** Slack is `innerH − span`, where span is the union DOCUMENT extent of the must-see facts; a negative slack means no scroll offset shows them all, so the auto-scroll cannot rescue it. Reported by `scripts/measure-simultaneity.mjs`.
  ...
  **The span is fully DERIVED as of 2026-07-27** — no term is sampled any more.
  ...
  - **deskHeight ≤ 156.5px** — ***WITHDRAWN 2026-07-27 (D1). The correct worst case is 161.5px.*** The stage row still saturates at the first staged card, but its HEIGHT depends on what is staged: a **joker face is 78.5px against every other card's 73.5px** (its own aspect is 1.548 against 1.45). Diagnosed by decomposing the desk into its child rows rather than by correlating against attributes; confirmed by the staged card's own label (小王/大王 ⟺ 161.5, 黑桃2 ⟺ 156.5). **P(a 27-card hand contains a joker) = 69.0%**, which is exactly the 16/23 split the held-out run measured. The player chooses what to stage, so this is CONTENT-dependent (practice 18) and the term needs a proved worst case, not a mean: **161.5px timed, 153.5px untimed.**
