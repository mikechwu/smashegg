...
- **G-SIM** (standing, new 2026-07-27, **replaces G-FOLD for the phone**): **a change must not reduce the worst-case SIMULTANEITY SLACK below the recorded baseline for that `viewport@theme`.** Slack is `innerH − span`, where span is the union DOCUMENT extent of the must-see facts; a negative slack means no scroll offset shows them all, so the auto-scroll cannot rescue it. Reported by `scripts/measure-simultaneity.mjs`.
  ...
  **The span is DERIVED, not sampled** (`scripts/derive-span.mjs`): `span = fanHeight + deskHeight + K`, with **K = 198.5px** when the trick well renders and **66.0px** when the viewer leads and it is empty — each constant to 0.1px across 78 measured states. So the span's bound reduces to the bounds of its two variable terms. `deskHeight ≤ 148.5px` is **structural**. `fanHeight` is **not yet bounded**: observed 230.8–316px, every value on the known 21.3px lattice. That remaining unknown is the whole of what keeps these worst cases sampled rather than proved.
