# Guandan (Guandan) — Implementation-Grade Rules Specification
...
- **Turn direction:** play proceeds to the next seat in a fixed rotation. Chinese play is traditionally counter-clockwise; online implementations vary and it has no logical effect except on tie-break rules that reference "next player" (the next seat (in turn direction)). **VARIANT** `turnDirection`, default `counterclockwise`. **UNCERTAIN** (sources rarely state it explicitly; pick one and be consistent — "the next seat (in turn direction)" below always means "next player in turn direction").
...
- **Level (the level (rank)):** a rank in `2..A` per team.
...
- **Wild card (the wild rule / wild card):** exactly the two cards `(♥, currentLevelRank)`. See §4.
...
5. **Current level of a hand (the level (rank) for this hand):** the level of the team whose member was **1st finisher (1st finisher) of the previous hand** — i.e., the "declarer" team plays *their* level. For the **first hand**, the current level is **2**. **CORE**
...
|---|------|-------|------------|--------------------|
| 1 | Single (single) | 1 | any card | `levelValue` |
| 2 | Pair (pair) | 2 | two equal ranks (incl. SJ+SJ, BJ+BJ) | `levelValue` |
| 3 | Triple (triple) | 3 | three equal ranks (jokers impossible) | `levelValue` |
| 4 | Full house (full house) | 5 | triple + pair, different ranks | `levelValue` of the **triple only**; pair irrelevant to comparison **CORE** |
| 5 | Straight (straight) | **exactly 5** | 5 consecutive natural ranks, mixed suits, no jokers | `naturalValue` of top card (A-low straight has top = 5) |
| 6 | Tube (three consecutive pairs / wooden board) | 6 | 3 consecutive pairs (e.g., `445566`), no jokers | `naturalValue` of top pair (`AA2233` top = 3; lowest) |
| 7 | Plate (plate / two consecutive triples) | 6 | 2 consecutive triples (e.g., `555666`), no jokers | `naturalValue` of top triple (`AAA222` top = 2; lowest) |
| 8 | Bomb (bomb) | 4–10 | n cards of one rank, n ≥ 4 (jokers excluded) | (count, `levelValue`) — see §3.11 |
| 9 | Straight flush (straight flush / rocket) | 5 | straight, all one suit | `naturalValue` of top card; is a **bomb** for beating purposes |
| 10 | Joker bomb (four heavenly kings / joker bomb) | 4 | exactly SJ,SJ,BJ,BJ | unique; highest |
...
- **3.7** Straight flush with wild(s) still **counts as a straight flush** (bomb rank preserved). **CORE** per official the wild rule rules; minor house variants demote it to a plain straight — config `wildStraightFlushIsBomb`, default `true`.
...
## 4. Wild Card the wild rule — Definition, Semantics, and the Validation Problem
...
1. **First lead of the match:** **VARIANT** `firstLeadMethod`: `random` (online default), `drawCard` (offline: a card is drawn/turned up during the deal; the player who receives it leads — pagat), or `fixedSeat`. Default `random`. Subsequent hands: lead is determined by tribute rules (§7.5) or by 1st finisher on anti-tribute/no-tribute.
...
6. **jiefeng (jiefeng, "receiving the wind"):** if the trick winner **emptied their hand with that winning play** (nobody beat their final cards), the lead for the next trick passes to their **partner** — confirmed (Wikipedia: "if the player who made the last discard now has no cards, the lead moves to their partner"). **CORE**. Rare house variant gives it to the next player: config `jiefengRecipient`, default `partner` (owner-confirmed, round 3 — including the exact condition below: jiefeng fires only when the winner emptied their hand with the winning final play; a beaten final play means no jiefeng).
   - Exact condition: (a) player P plays their last cards, (b) that play wins the trick (all others pass / no one beats it). If P's final play is beaten, no jiefeng — normal rules continue and P is simply skipped thereafter.
   - The partner is always still active when jiefeng fires: if P's partner had already finished, P's finish makes P either the **2nd** finisher (teammates 1st+2nd, 1-2 finish) or the **3rd** finisher — in both cases the hand ends immediately per §5.8, before any lead is needed. Engine may assert this invariant.
7. **Finishing order terminology:** 1st finisher (1st out), 2nd finisher (2nd), 3rd finisher (3rd), 4th finisher (4th / "Dweller"). **CORE**
8. **Hand end:** immediately when the 1st and 2nd finishers are teammates (1-2 finish; remaining play is moot), otherwise immediately when the 3rd player finishes (the remaining player is 4th finisher; their leftover cards have **no further rules significance** — they only fix the finishing order and are shuffled into the next deal; tribute is computed from the **newly dealt** hand, §7.2/§9.19). Hand may therefore end mid-trick. **CORE**

---

## 6. Hand Scoring & Level Upgrade

1. **Result classes** (by the winning team = 1st finisher's team):
   - Partner finished **2nd** (1-2 finish / "1-2"): **+3 levels**. Hand ends the moment the 2nd teammate goes out.
   - Partner finished **3rd** ("1-3"): **+2 levels**.
   - Partner finished **4th** ("1-4" / single-up finish): **+1 level**.
   **CORE**
...
4. **Winning the match at level A (passing A / playing A):**
   - A team can attempt A only in a hand **played at their level A** — i.e., they are the current-level ("declarer") team (§1.5). **CORE**
   - **Win condition (default):** their player is 1st finisher **and their partner is not 4th finisher** — i.e., a 1-2 or 1-3 result. A 1-4 result does **not** win the match (and grants no level, since A is the cap). **CORE per official/pagat**; casual variant: any 1st finisher at A wins — config `aWinPartnerNotLast`, default `true` (owner-confirmed, round 3).
...
## 7. Tribute (tribute / return tribute / anti-tribute)
...
1. **Who pays — CORE:**
   - Previous result **1-2** (losers took 3rd & 4th, "double loss / double-down"): **both losers pay one card each** (double tribute). The **1st finisher receives the higher-ranked** of the two tribute cards; **2nd finisher receives the lower**. (Mapping is by card rank, not by seat.)
   - Previous result **1-3 or 1-4** (single tribute): the **4th finisher pays one card to 1st finisher**. Note in a 1-4 result the 4th finisher is the 1st finisher's **own partner** and still pays (confirmed by pagat).
...
3. **Equal double tributes — VARIANT (genuine three-way cross-source conflict, source-verified 2026-07-13):** if the two tribute cards have equal rank, the assignment differs by source — official trial rules 「if card ranks are equal, tribute proceeds clockwise」; Tangrenyou/JJ allocate **randomly**; pagat: the **winners choose**. Config `equalTributeAssignment`: `seatOrder` (default, official) / `random` / `winnersChoose`.
   - **Engine definition of `seatOrder`:** the 1st finisher receives the card from the payer encountered first from 1st finisher **in turn direction** (i.e. 1st finisher's the next seat (in turn direction) among the two payers) — bound to `turnDirection`, **not** to absolute clockwise. The official text's literal clockwise is read as naming that ruleset's own turn direction (**UNCERTAIN** hair-split; it only matters if `turnDirection` is flipped, and it is one comparison in one function either way).
   - **Staging (both payments and both returns):** each payer/returner commits without seeing the other's card. The engine holds the first committed `payTribute` privately — it may emit a card-less `tributeCommitted{seat}` marker (mirroring the visible face-down handover) but no card — and once both are in, performs the assignment and emits both `tributePaid` events **atomically**; the two `tributeReturned` events are likewise atomic (official simultaneity device: 「both tribute payers reveal simultaneously」). No sequential information leak within either phase.
4. **Return tribute (return tribute) — CORE (official, confirmed):** each tribute receiver returns exactly one card with **`levelValue ≤ 10`** — a single formulation that is exactly equivalent to "natural rank 2–10 and not the current level rank; never jokers, level cards, or A/K/Q/J" — to the payer they received from (in double tribute the return follows the pairing established in §7.1/§7.3). Receiver **chooses freely among all qualifying cards** — a choice over a set, exactly like tribute payment; `legalActions` surfaces the eligible set for UI hinting and `applyAction` validates membership. **VARIANT** `returnTributeMaxRank`: default `10` (owner-confirmed, round 3; interpreted as `levelValue ≤ 10` — corroborated: a competitive explainer states that at level 10 the return must be ≤9, i.e. the current level card is not returnable even at face ≤10); pagat/Western variant `null` (any card, must merely differ from the tribute card). Edge case: if the receiver holds no qualifying card, the **official rules pin the fallback** — 「if all cards are above 10, return the lowest card」 (return the smallest card): config `returnNoLowCardPolicy`, default `lowestByLevelValue`, now **CORE-backed** rather than UNCERTAIN. The `anyCard` alternative is defined **only for this no-qualifying-card case**: a house-rule fallback where the receiver instead returns any card of their choice from the full hand.
5. **Who leads the new hand — mostly CORE, confirmed:**
   - Single tribute: the **tribute payer (4th finisher) leads**.
   - Double tribute: **the payer whose tribute card the 1st finisher received leads** — one rule covering both the unequal case (the higher tribute goes to 1st finisher, so its payer leads) and the equal case (whoever's card was assigned to 1st finisher under `equalTributeAssignment` leads). **CORE, hard-coded** (official: 「the player who paid tribute to the upper finisher leads the first trick」, unanimous across Chinese sources; pagat's fringe "payers agree who leads" is documented here but not implemented — the former `doubleTributeTieLead` config is removed as derivable).
   - No tribute owed (anti-tribute, or first hand): 1st finisher of the previous hand leads (first hand: §5.1).
6. **Anti-tribute (anti-tribute) — CORE (verified against the official trial rules, 2026-07-13):**
   - Condition: single tribute — cancelled iff the payer holds **both big jokers**; double tribute — cancelled (for **both** payers) iff the two payers **together** hold both big jokers (one each, or one holding both). Official: 「if the lower finisher or double-down side draws two big jokers before tribute, anti-tribute applies, no tribute is paid, and the upper finisher leads the first trick」. Computed from the **newly dealt** hands (§9.19) by the engine, which holds authoritative state.
   - Effect: no tribute, no return, and the previous hand's **1st finisher leads**.
   - **Mandatory public reveal (owner-pinned, round 2):** the qualifying big jokers are revealed to **all four players with holder attribution** (which player holds which joker) so everyone sees why tribute was skipped — and **nothing else** from those hands is shown. Matches physical play (showing the jokers is how entitlement is proven) and platform practice (JJ/Tangrenyou display them in the UI: 「the big jokers used for anti-tribute must be revealed in the UI」). Engine: emit a public `antiTribute{reveals:[{seat,card}]}` event; `viewEvent` returns it whole to every seat. **UNCERTAIN** only in the narrow sense that the retrieved official clause text does not spell out the reveal step; the reveal itself is required behavior here in every mode.
   - Invocation: **automatic/mandatory upon the condition** — every source found states it declaratively; targeted searches for a decline flow (decline anti-tribute / can decline anti-tribute) found none (null result), and declining is strategically dominated (it would surrender a big joker). **VARIANT** `antiTributeMode`: `auto` (default) / `optional` (future-proofing only; no known platform implements it). The public reveal always accompanies **invocation**, in either mode.
   - **`optional`-mode state machine (engine-defined — UNCERTAIN by construction, no platform reference exists):** after the deal, if the condition holds, the qualifying payer(s) act *before* any tribute is paid via `{type:'antiTributeDecision', invoke}`; **nothing is revealed while deciding**. Double tribute with one big joker each: invoking requires **both** payers to opt in (both must reveal); a single holder of both jokers decides alone. Invoked → the standard public reveal fires, no tribute/return, previous 1st finisher leads. Declined (or not unanimously invoked) → nothing is revealed and the normal tribute flow proceeds. `defaultAction` for the decision = `invoke` (mirrors `auto`; preserves the liveness obligation on timeout).
...
## 8. Reporting (low-card alert) & Table Talk
...
3. **Table talk:** no signaling, no revealing hand contents, partners may not coordinate beyond legal plays. Not engine-enforceable; relevant to UI/chat policy only. Tribute/return cards and anti-tribute joker reveals are the only sanctioned information leaks. **CORE** (as a policy statement).
...
3. **Hand may end mid-trick** (3rd finisher, or teammates 1st+2nd) — abort the trick immediately; the 4th finisher's leftover cards matter only for fixing the finishing order (tribute uses the next hand's fresh deal, §7.2/§9.19). **CORE**
4. **jiefeng invariant:** the finisher's partner is always active when jiefeng fires (proof in §5.6); assert it.
5. **Four jokers & tribute:** tribute is a single card, so a joker bomb is never "paid"; one big joker can be paid; both big jokers in the payer set ⇒ anti-tribute (§7.6).
...
19. **Tribute forced-rank computation** must exclude wilds but include non-heart level cards and single jokers; run it — and the anti-tribute big-joker check (§7.6) — on the payer's **newly dealt 27-card hand**, after the deal and before any play. The previous hand's leftover cards are never consulted. The result is a *rank*; the payer chooses which concrete card of that rank to give (§7.2).
...
| `equalTributeAssignment` | enum | `seatOrder` (official), `random` (Tangrenyou/JJ), `winnersChoose` (pagat) | `seatOrder` | ✓ | §7.3 |
...
Removed in v1.2: `doubleTributeTieLead` (the lead rule is unanimous and derivable — the payer whose card 1st finisher received leads, §7.5); `doubleTributeTieAward` renamed/expanded to `equalTributeAssignment`; `antiTributeAutomatic` renamed to `antiTributeMode`.
...
- **Competitive Guandan Rules (Trial)** (General Administration of Sport of China, Chess and Card Games Administrative Center, 2022/2023): Article 10, Tribute and Return Tribute retrieved verbatim from two agreeing copies — [sohu.com](https://www.sohu.com/a/621481154_121124679), [52hrtt.com](https://www.52hrtt.com/ny/n/w/info/F1702965842341) (tribute/return/anti-tribute clauses; fetched 2026-07-13; full verification report with per-question source tables: docs/research/guandan-tribute-verification.md)
...
- [Guan Dan — pagat.com](https://www.pagat.com/climbing/guan_dan.html) (tribute/return/anti-tribute mechanics, 1-4 4th finisher-pays-1st finisher, A-level 3-attempt demotion to 2, ≤10-card report-on-request, first-deal drawn-card lead, wilds-not-jokers)
- [Guandancompetition rules (NUIST, official-style PDF)](https://gh.nuist.edu.cn/_upload/article/files/e2/4e/9a5343d0450a9580fd4c1f46fc0b/290d8299-e923-4966-a485-b0269c7e11fd.pdf) and [playing cards（Guandan）competition rules (SEU PDF)](https://ddgh.seu.edu.cn/_upload/article/files/44/c8/f455e1d04d2a998e40454931740a/4f853bb4-29b9-45dc-9c56-7627ed4c9726.pdf) (four jokers are highest, bomb ladder, tribute/return tribute/anti-tribute)
- [Guandan rules — Zhihu](https://zhuanlan.zhihu.com/p/5480576176), [Lianzhong Guandan introduction](https://www.ourgame.com/game/game-intro-new/df/sc/h2g/egglaying_002.html), [Tangrenyou Guandan rules](https://guandan.uc55.com/Rule/Detail/201801_000209.html) (return tribute≤10, equal-tribute the next seat (in turn direction) conventions)
