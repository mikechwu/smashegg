...
2. **Validate (V):** for the declared `(type, keyRank, suit?)`, construct the **required multiset** of card identities:
   ...
   - Special checks: standalone wilds rule (§4.2) — if `type ∈ {single, pair}` and `S` is all wilds, `keyRank` must be the level rank; joker-containing types per §2.2; §3.6 five-of-a-kind guard; **§3.8 guard** — if `type = straight` and `S` contains **no wilds** and all five cards are one suit, reject under default `allowUnderDeclareStraightFlush = false` (the set must be declared as a straight flush). **Wild policy for §3.8:** when wilds are present the declaration is free — assigning a wild an off-suit identity to form a plain straight is legal substitution (§4.1), so a wild-completed suited set may be declared either as SF or as a plain straight; the guard applies only to fully-natural one-suit sets.
3. **Generate (G):** iterate over **templates**, not subsets:
   ...
   - **Canonical form** is `(type, size, keyRank, suit)` with `suit` present **only for straight flushes**: SF validation requires the declared suit, while beat-comparison ignores it (§2.3). For generation, **dedupe by the suit-blind projection** `(type, size, keyRank, isSF)` — e.g., `3♠4♠5♠6♠ + wild` yields SF-top-7 *and* SF-top-6 (wild as 2♠) *and* the corresponding plain-straight interpretations (wild assigned off-suit — legal per the §4.4.2 wild policy); emit each distinct projection once. When beating a prior play, filter by the beats-relation first, which prunes most templates.
...
18. **Ambiguous selections** (wilds): require declared canonical form when ≥ 2 interpretations exist (§4.4.4) — e.g., `2♠3♠4♠5♠+wild` = SF-6♠ / SF-A-low / straight; the declaration binds what followers must beat.
