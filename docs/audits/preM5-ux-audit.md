...
**P2** friction · **P3** polish. Must-see tags: [T] whose-turn · [L] the level (rank) ·
[W] wild the wild (heart level card) · [B] what to beat · [H] hand · [G] legal hints · [C] card counts ·
...
- **F3 · Raw error code leaks; pre-seat pickers rejected; error is sticky and duplicated.**
  ...
  **「action rejected(room.notSeated)」** — the raw semantic code, in every locale ("Action
...
- **F5 · No partner/team cue on the plates.** Nothing marks A-Hua as MY partner; the our team/opponents
...
- **F6 · The wild (the wild (heart level card)) is never stated on the table** — only the wild corner marker on a
  wild card you happen to hold. If you don't hold one, nothing says "the wild is heart 2."
  Needs an always-present wild indicator by the level. [W]
- **F7 · The level (the level (rank)) is present but not headlined.** Conveyed by the level rail (current
  rank at the BOTTOM of a 13-row ladder) + a one-time feed line. The brief calls it "the
  most under-served critical fact." Wants a persistent, prominent playing X headline (with the
  wild), esp. on mobile. [L]
- **F8 · Whose-turn on YOUR seat is spectator-phrased.** When it's your turn the dominant
  center text reads "waiting for you to lead", not "your turn." The only real
  cues are the enabled Play button + a thin plate border (easy to miss at 390px). Research
...
- **F9 · Legal-play hints invisible during a normal turn** (owner decision). Facing
  "A-Qiang・single9", the fan shows all 27 cards with no cue which are legal. Per-card `glow` is
  ...
  "must beat single9" line by the action bar, or highlighting beating singles when nothing is
  selected. Not a silent fix. [G]
- **F11 · Card counts have no low-count (low-card alert) emphasis** (confirmed from `SeatPlate.tsx`: the
  count chip is value-independent). A seat at 2 cards looks identical to one at 27. Research
  (official low-card alert rule): ≤10 is the urgency line — escalate the count (size/border/icon),
  sharper at 1–2. [C]
...
- **F2 · Game-name glyph:** zh-Hant uses Guandan; strict Traditional is Guandan. Widely accepted, likely
...
picker; the ceremony (the draw ceremony (flip-to-lead), clear + tappable skip); the level STATE — real levels /
A-attempts / suspension (NOTE: Phase B RETIRED the LevelRail ladder and folded this into the
headline team badges; the ladder visualisation is deferred); the wild **wild marker** (legible,
localizes wild/w); card counts on every plate; the
trick well ("name・combination"); **CCW turn order** (south→east→north→west, correct); the hand fan
...
clock distinction (opening-hand thinking label; dims under the ceremony).

## Deliberately NOT re-driven live (recently verified M3/M4; Phase-B re-verify against the ring)
Wild multi-reading chooser (a lone wild correctly plays as single2, no chooser); tribute /
anti-tribute / jiefeng panels; match-end result overlay; a live "1–2 cards" state (inferred from
