...
**Scoring / clamp / A-win (`levels.ts`, consumed by `index.ts`):** §6.1 +3/+2/+1 from partner place (incl. truncated finish orders, absent partner = 4th → +1); §6.3 A clamp, overshoot only when target **past** A (`K+1` on A ≠ overshoot; `K+2`/`K+3` overshoot); `overshootWinsGame` true/false; A-win = declarer + own A + `currentLevel === 'A'` + not suspended; `aWinPartnerNotLast` true (1-2/1-3) / false (any 头游); non-declarer at A cannot 过A; at-A 1-4 gives no level gain but consumes the attempt.
...
**Spot-check, tribute:** single tribute leader = payer (末游); double unequal → higher card to 头游, leader = that payer; double equal `seatOrder` → first payer from 头游 via `nextSeat`/`turnDirection`; double equal `random` → PRNG choice; anti-tribute → 头游 leads; leader always "payer of 头游's card" after return resolve.
...
- Trick/接风/hand-end mid-trick — not in scope; trusted only as finishOrder producer into scoring.
