...
- **M0 scaffold built and locally verified** (4 scaffold agents + orchestrator verification pass):
  ...
  - Remaining for the M0 gate: owner's four SETUP.md human steps (Cloudflare account + subdomain, `wrangler login`, CI API token → `gh secret set`, `gh auth`) → create GitHub repo, push, CI green → `wrangler deploy` → **G-COMPOSE + G-ALARM on the real free tier** → record workers.dev URL as `WORKER_URL` repo variable for the deploy smoke step.
