...
| `stripW` | `definitional` | — | **`lacquer` only** | lacquer | 0.42. The withdrawn cinnabar-court requested 0.841. Every span figure in this model is a lacquer figure. |
| `stripCeiling` | `structural` | — | any theme, at a given card and depth floor | derived, so it moves with both | 0.447 at the shipped card and depth floor. A theme requests a reveal; the framework owns this ceiling. |
